...
k_soft = 1; k_hard = 5; % spring const for soft/hard tissue
k = [zeros(1, N0/2) + k_soft, zeros(1, N0/2) + k_hard]; % vector of spring const
a = zeros(1, N0) + a0; % equilibrium length
d = l0*0.8; % half occupation constant
n = 30; % Hill constant
dp = 0.005; % max division probability
xp = 0.005; % death probability
...
while t < tmax
    ...
    if mod(round(t,1), 0.1) == 0
        ...
        while i<length(x)
            ...
            %plot([x(i), x(i)], [1, 2], 'k')
            %hold on
            plot(polyshape([x(i) 1 x(i+1) 2],[x(i+1) 1 x(i) 2]))
            hold on
            i=i+1;
        end
        drawnow
